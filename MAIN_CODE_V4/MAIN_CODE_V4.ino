...
void loop() {
  ...
  if (getAlarmState() == 1) {    
    ...
    if (inoutResult == 2) {         // got home late
      //Serial.println("in");
    
    // 제한시간(5분) 이내 rfid 태그 코드
      if (!limitVal){
        limitS = getLimitStart();
        limitVal = true;
      }
      ...
      if (limitS + limitTime == LimitCheck()){
        uart = 2;
        limitVal = false;
      }
  ...
  if (getAlarmState() == 0) {
    ...
    if (inoutResult == 2) {   // got home in time
      ...
      if (limitS + limitTime == LimitCheck()){   // Access time over
        uart = 2;
        limitVal = false;
      }
    }  
